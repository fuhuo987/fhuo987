...
* ## axios
* ### axios简介
axios是一个基于promise实现的网络请求库，可以运行在浏览器和nodejs中。在nodejs中基于http模块实现，在浏览器中基于XMLHttpRequest实现

原生的XMLHttpRequest在编写异步请求时比较繁琐。在创建XMLHttpRequest实例时需要写很多兼容代码，原生的XMLHttpRequest功能比较简陋，请求和响应的处理会有很多的冗余代码，多个异步请求容易造成回调地狱

axios 提供了更简洁的 API、更好的错误处理、自动转换 JSON 数据、请求取消等特性，使用起来更方便且功能更强大 
* **
* ### axios的基本使用
```
axios({
url:'地址',}).them(res=>{console.log(res);})
```
默认是使用get方式进行请求
...
```
axios会自动指定header的```'Content-Type```为```"application/json```，不需要像fetch一样声明，而且fetch的json参数需要手动序列化
...
因为axios也是基于promise，所有它和fetch一样能写成async/await的语法
* **
* ### 拦截器
...
在底层的实现上，vue把模板编译成DOM渲染函数，vue能智能地计算出最少需要渲染多少个组件，并把DOM的操作次数减少到最小

**文本**
数据绑定最常见的形式就是使用双大括号语法的文本插值
```
...
在```<template></template>```里写HTML内容

**原始HTML**
双大括号会将数据解释为普通文本而非HTML代码，为了输出真正的HTML代码，需要使用```v-html```指令
```
<div>{{tp}}</div>    //相当于innerText
<div v-html='tp'></div>    //相当于innerHTML
```

**属性 Attribute**
双大括号语法不能在HTML属性中使用，但是可以使用```v-bind```指令
```
<div v-bind:class="zheshilei"></div>
```
```v-bind```可以简写成```:```，也就是说上面的代码可以这么写
```
<div :class="zheshilei"></div>
```
<br>

**使用js表达式**
在模板中，我们一直都只绑定简单的property键值，vue.js提供了完全的js表达式支持。需要注意的是，每个绑定只能包含单个表达式
```
{{num+1}}
{{flag?'YES':'NO'}}
```
* **
* ### 条件渲染
```v-if```指令用于条件性渲染一些内容，这些内容只会在指令的表达式返回```true```的时候渲染
```
<p v-if='flag'>123456789</p>
```
```
data(){
return{flag:true}
...
还可以使用```v-else```指令来表示```v-if```的else块
```
<p v-if='flag'>123456789</p>
<p v-else>987654321</p>
```
<br>

另一个用来条件性展示元素的选项是```v-show```指令
```
<p v-show='flag'>1234</p>
```
<br>

```v-if```是“真正”的条件渲染，因为它会确保在切换过程中条件块内的事件监听器和子组件适当的被销毁和重建

```v-if```是惰性的，初始渲染时条件为假那就什么都不做，只有到第一次条件为真时才会开始渲染条件块

相比之下，```v-show```就要简单很多，不管条件是什么，元素都会被渲染，只是简单地基于CSS进行切换

```v-if```具有更好的切换开销，```v-show```具有更高的初始渲染开销，如果要频繁切换就用```v-show```，反之用```v-if```
* **
* ### 列表渲染
可以使用```v-for```基于一个数组来渲染一个列表，它需要使用```item in items```形式的特殊语法，其中items时源数据数组，item是被迭代的数组元素的别名
...
data(){
retrun{
    list:[{msg:'fuhuo'},{msg:'1234'}]
    }
}
```
<br>

当vue正在更新用```v-for```渲染的元素列表时，它默认使用“就地更新”的策略。如果数据项的顺序被改变，它会就地更新每个元素，确保它们在每个索引位置被正确渲染

为了给vue提示以便它能跟踪每一个节点的身份，需要给每项提供一个唯一的属性```key```
```
<div v-for="item in items":key="item.id"></div>
```
如果你的数组里没有id这种唯一的东西，就可以这么写
 ```
 <div v-for="(item,index) in items":key="index"></div>
 ```
 这是因为数组中元素的下标也是唯一的。但是一般来说数据都要有id
* **
* ### 监听事件
可以用```v-on```指令（通常缩写为```@```符号）来监听DOM事件，并在触发事件时执行一些JS。语法为```v-on:click="方法名"```或者```@click="方法名"```

许多事件处理逻辑会比较复杂，直接把JS代码写在```v-on```是不可行的，```v-on```还可以接收一个需要调用方法的名称

除此之外还有事件传递参数，如：
```
<botton @click='say(hi)'></botton>
```
```
methods:{
...
**修饰符**
```.lazy```： 一般情况下，```v-model```在每次```input```事件触发后将输入框的值和数据进行同步，增加该修饰符就会变成在```change```事件后进行同步（如回车或失去焦点）

```.trim```： 自动过滤用户输入首尾的空格
* **
* ### 组件基础
**单文件组件**
vue单文件组件（又叫```*.vue````文件，缩写SFC）是一种特殊的文件格式，允许将vue组件的模板、逻辑和样式封装在单个文件中

**加载组件**

1. 引入组件```import MyComponentVue from './components/MyComponentVue.vue'```，写在```<script>```里面
2. 挂载组件```components:{MyComponentVue}```，写```export default```里面
3. 显示组件```<my-componentvue/>```或者```<MyComponentVue/>```，写```<template>```里面

如果在```<style>```标签里加入```scoped```属性，就代表这里面的样式只在当前组件生效

组件的组织通常会以一颗嵌套组件树的形式来组织

组件和组件之间是要存在交互的，否则完全没关系那组件就没什么意义了

```prop```是可以在组件上注册一些自定义attribute

如果父级写了```<MyConponent :title='biaoti'/>```想把```biaoti```这个变量的值传给子级的```title```属性，那么子级需要做接收，在```export default```里写```props:{title:{type:String,default:'默认值'}}```

prop传递参数是没有类型限制的，注意，数组和对象必须使用函数进行返回，即：
```
default:function(){
return []
}
...
上面是从父级传到子级，想要实现反向传递数据可以利用自定义事件来实现```$emit```

```
<botton @click='send'>点击传递<botton>
```
```
data(){
...
method:{
send(){}
this.$emit('onEvent',this.msg)   //参数一是字符串，参数二是要传递的数据
}
```
父组件里就把那个字符串当事件来看
```<MyComponentVue @onEvent="getdata"/>```
```
method:{
getdata(data){
console.log(data)
...
每个组件在被创建时都要经过一系列初始化过程，如需要设置数据监听、编译模板、将实例挂载到DOM并在数据变化时更新DOM等。在这个过程中会运行一些叫**生命周期钩子**的函数，给了用户在不同阶段添加自己代码的机会
